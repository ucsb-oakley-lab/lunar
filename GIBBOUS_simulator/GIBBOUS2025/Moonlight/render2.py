...
def run_simulation(year, default_speed_factor=10000):
    ...
    selected_phase = input("Enter the starting moon phase: ").strip().title()
    
    # Set the start date to the approximate date of the selected phase in the given year
    target_year_date = PHASE_DATES.get(selected_phase, datetime.datetime(year, 1, 1)).replace(year=year)
    
    # Get user input for a custom lunar cycle length
    try:
        custom_cycle_length = float(input(f"Enter the desired lunar cycle length in days (default is {DEFAULT_LUNAR_CYCLE_LENGTH} days): ").strip())
    except ValueError:
    ...
    while True:
        ...
        # Print the current azimuth angle to the terminal
        print(f"Current Azimuth Angle: {az:.2f} degrees")
        ...
        # Display simulated date and time at the top of the frame
        cv2.putText(frame, f"Simulated Date: {simulated_date.strftime('%Y-%m-%d %H:%M:%S')}", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        # Display phase, altitude, and azimuth information
...
run_simulation(year=2018, default_speed_factor=10000)
