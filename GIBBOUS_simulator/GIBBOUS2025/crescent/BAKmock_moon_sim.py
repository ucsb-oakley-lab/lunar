...
BRIGHTNESS_GAMMA = 1.0  # 1.0 = linear, >1.0 makes full moon relatively brighter
			# 2.0 is estimate for natural moonlight, but should be measured
...
def get_num_phases(target_cycle_length):
    ...
    for phase in LUNAR_PHASES:
        ...
        if scalar >= 1:
            new_length = math.floor(scalar * base_length) if MoonPhaseChecker[phase] == 'x' \
                else math.ceil(scalar * base_length)
        else:
            new_length = math.ceil(scalar * base_length) if MoonPhaseChecker[phase] == 'x' \
                else math.floor(scalar * base_length)
...
def simulation_loop(schedule, cycle_start_date, user_cycle_length, speed_factor, hex_color, stop_event):
    """Run the moonlight + sunlight simulation on the main thread, printing status each step."""
    ...
    print(f"[Simulation] Starting from {cycle_start_date}, running for {user_cycle_length} days "
          f"({speed_factor}x speed). Close the plot window to stop.")
    ...
    while not stop_event.is_set() and sim_time < cycle_end_time:
        ...
        print(f"[Sim {sim_time.strftime('%Y-%m-%d %H:%M')}] "
              f"{phase_name} (brightness={brightness:.2f})")
...
def simulation_loop_oled(schedule, cycle_start_date, user_cycle_length, speed_factor, hex_color, stop_event):
    """Run the moonlight + sunlight simulation on the Pi using the Waveshare OLED."""
    ...
    print(f"[OLED Simulation] Starting from {cycle_start_date}, running for {user_cycle_length} days "
          f"({speed_factor}x speed). Press Ctrl+C to stop.")
    ...
    while not stop_event.is_set() and sim_time < cycle_end_time:
        ...
        print(f"[OLED {sim_time.strftime('%Y-%m-%d %H:%M')}] "
              f"{phase_name} (brightness={brightness:.2f})")

        # Draw the color on the OLED
        try:
...
def plot_moon_schedule_times(schedule):
    """Plot moonrise and moonset times for each day in the schedule."""
...
def plot_moon_phase_angle(schedule):
    """Plot phase angle over the lunar cycle."""
...
def plot_moon_schedule_phases(schedule):
    """Scatter plot showing lunar phase for each day in the schedule."""
    days, phase_indices = [], []
    for entry in schedule:
        days.append(entry['day'] + 1)
...
def handle_command(cmd, arg, stop_event, state):
    if cmd == 'pt':
        ...
        print("Unknown command. Try: pt, pp, pang, change, start, startoled, status, q")
