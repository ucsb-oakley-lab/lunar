...
def plot_contours(file_path, glare=False, zoomx=None, zoomy=None):
    """
    Plots frame vs. cY from a tab-delimited file containing contour data.
    ...
    if glare and 'glare' in data.columns:
        ...
        plt.scatter(frame, cY, marker='o', s=1, c=glare_colors)
    else:
        plt.scatter(frame, cY, marker='o', s=1)

    plt.xlabel('Frame')
    plt.ylabel('cY')
    plt.title('Frame vs. cY')
    plt.grid(True)
    plt.show()
    ...
    if zoomx or zoomy:
        ...
        plt.title(f'Frame vs. cY {zoom_title}')
        plt.grid(True)
        plt.show()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Plot frame vs. cY from a contour data file.")
